...
class FHIR_TestScript_setup_action_operation(models.Model):
    ...
    description = FHIR_primitive_StringField(null=True, blank=True, )
    class AcceptChoices(models.TextChoices): MIME_TYPE_TO_ACCEPT_IN_THE_PAYLOAD_OF_THE_RESPONSE,_WITH_CHARSET_ETC = 'Mime type to accept in the payload of the response, with charset etc', 'Mime type to accept in the payload of the response, with charset etc'; 
    accept = FHIR_primitive_CodeField(choices=AcceptChoices.choices, null=True, blank=True, )
    class ContenttypeChoices(models.TextChoices): MIME_TYPE_OF_THE_REQUEST_PAYLOAD_CONTENTS,_WITH_CHARSET_ETC = 'Mime type of the request payload contents, with charset etc', 'Mime type of the request payload contents, with charset etc'; 
...
class FHIR_TestScript_setup_action_assert(models.Model):
    ...
    class ContenttypeChoices(models.TextChoices): MIME_TYPE_TO_COMPARE_AGAINST_THE_'CONTENT_TYPE'_HEADER = 'Mime type to compare against the 'Content-Type' header', 'Mime type to compare against the 'content-type' header'; 
    contentType = FHIR_primitive_CodeField(choices=ContenttypeChoices.choices, null=True, blank=True, )
