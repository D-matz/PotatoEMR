...
class FHIR_Endpoint_payload_mimeType(models.Model):
    ...
    class MimetypeChoices(models.TextChoices): MIMETYPE_TO_SEND._IF_NOT_SPECIFIED,_THE_CONTENT_COULD_BE_ANYTHING_INCLUDING_NO_PAYLOAD,_IF_THE_CONNECTIONTYPE_DEFINED_THIS = 'Mimetype to send. If not specified, the content could be anything (including no payload, if the connectionType defined this)', 'Mimetype to send. if not specified, the content could be anything (including no payload, if the connectiontype defined this)'; 
